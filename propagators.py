...
def prop_FC(csp, newVar=None):
    ...
    for c in constraints:
        if c.get_n_unasgn() == 1:
            ...
            for d in unasgnd_var.cur_domain():
                if not c.check([val if val != None else d for val in vals]):
                    pruned_vars.append((unasgnd_var, d))
                    unasgnd_var.prune_value(d)
